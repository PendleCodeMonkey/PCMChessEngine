...
{
	/// <summary>
	/// Implementation of the <see cref="Zobrist"/> class.
	/// </summary>
	/// <remarks>
	...
	public class Zobrist
	{
		...
		private static readonly ulong[] _zobristRnd64 = {
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Zobrist"/> class.
		/// </summary>
		public Zobrist()
		{
		}
		...
		public static ulong WhiteMoveKey => 0xf8d626aaaf278509L;
		public static ulong WhiteKingSideCastlingKey => 0x31d71dce64b2c310L;
		public static ulong WhiteQueenSideCastlingKey => 0xf165b587df898190L;
		public static ulong BlackKingSideCastlingKey => 0xa57e6339dd2cf3a0L;
		public static ulong BlackQueenSideCastlingKey => 0x1ef6e6dbb1961ec9L;
		public static ulong[] EnPassantColumnKeys { get; } = { 0x70cc73d90bc26e24L, 0xe21a6b35df0c3ad7L, 0x3a93d8b2806962L, 0x1c99ded33cb890a1L, 0xcf3145de0add4289L, 0xd0e4427a5514fb72L, 0x77c621cc9fb3a483L, 0x67a34dac4356550bL };
		...
		/// <summary>
		/// Get the key for a specified piece at a given location on the board.
		/// </summary>
		/// <param name="loc">The zero-based location on the board.</param>
		/// <param name="pieceAtLoc">A character representing the piece.</param>
		/// <returns>The key for the specified piece at the given location on the board.</returns>
		public static ulong GetKeyForSquare(int loc, char pieceAtLoc)
		{
			string pieces = "pPnNbBrRqQkK";

			int pieceIndex = pieces.IndexOf(pieceAtLoc);
			if (pieceIndex >= 0)
			{
				return _zobristRnd64[(pieceIndex * 64) + loc];
			}
		...
		/// <summary>
		/// Get the key representing a move by the specified piece.
		/// </summary>
		/// <param name="from_loc">The location the piece is moing from.</param>
		/// <param name="to_loc">The location the piece is moving to.</param>
		/// <param name="piece">A character representing the piece.</param>
		/// <returns>The key for the move made by the specified piece</returns>
		public static ulong GetKeyForMove(int from_loc, int to_loc, char piece)
		{
			return GetKeyForSquare(from_loc, piece) ^ GetKeyForSquare(to_loc, piece);
		}

		/// <summary>
		/// Get the key for the current state of the board.
		/// </summary>
		/// <param name="b">The state of the board.</param>
		/// <returns>The key for the current state of the board.</returns>
		public static ulong GetKeyForBoard(Board b)
		{
			ulong key = 0;

			// XOR in the key for each square on the board.
			for (int loc = 0; loc < 64; loc++)
			{
				key ^= GetKeyForSquare(loc, b.GetPieceAt(loc));
			}

			// XOR in the castling keys
			if (b.WhiteCastleKingside)
			{
				key ^= WhiteKingSideCastlingKey;
			}
			if (b.WhiteCastleQueenside)
			{
				key ^= WhiteQueenSideCastlingKey;
			}
			if (b.BlackCastleKingside)
			{
				key ^= BlackKingSideCastlingKey;
			}
			if (b.BlackCastleQueenside)
			{
				key ^= BlackQueenSideCastlingKey;
			}

			// XOR in the en-passant key (if required)
			if (b.EnPassantLoc != -1)
			{
				// Determine if the en-passant is valid (by checking for pawns in key locations) and
				// only apply the en-pasant key when it is valid.
				var file = BitboardUtils.GetFile(b.EnPassantLoc);
				if (b.WhiteToMove)
				{
					if ((file > 0 && b.GetPieceAt((4 * 8) + file - 1) == 'P') ||
							(file < 7 && b.GetPieceAt((4 * 8) + file + 1) == 'P'))
					{
						key ^= EnPassantColumnKeys[BitboardUtils.GetFile(b.EnPassantLoc)];
					}
				}
				else
				{
					if ((file > 0 && b.GetPieceAt((3 * 8) + file - 1) == 'p') ||
							(file < 7 && b.GetPieceAt((3 * 8) + file + 1) == 'p'))
					{
						key ^= EnPassantColumnKeys[BitboardUtils.GetFile(b.EnPassantLoc)];
					}
				}
			...
			// XOR in the "white move" key if it is white's turn to move.
			if (b.WhiteToMove)
			{
				key ^= WhiteMoveKey;
			}
